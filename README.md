...
**سامانه‌ی یکپارچه‌ی نگهداری و تعمیرات صنعتی**

نسخه‌ی فارسی · [English](README.en.md) · [صفحه‌ی معرفی](https://m0000hamad.github.io/davam-showcase/)

![نسخه](https://img.shields.io/badge/نسخه-6.35.0-4668E2)
...
دوام یک سامانه‌ی **CMMS/PMS** تحت وب برای هر سازمانی است که دارایی فیزیکی
دارد و باید نگهش دارد — کارخانه، خط تولید، نیروگاه، ناوگان، پالایشگاه یا
دکل حفاری.

منطق سامانه به هیچ صنعت خاصی گره نخورده: **تجهیز، برنامه‌ی نگهداری،
دستور کار، خرابی، قطعه و شاخص** مفاهیمی هستند که در هر صنعت
دارایی‌محور یکسان‌اند. آنچه دوام را متمایز می‌کند این است که این مفاهیم
را با انضباط واقعی پیاده کرده، نه با یک فرم خالی.

سامانه در **سخت‌ترین محیط ممکن آبدیده شده**: صنعت حفاری، جایی که توقف
تجهیز مستقیم به هزینه‌ی روزانه تبدیل می‌شود. امروز روی سه مجموعه در
بهره‌برداری است و در **۱۲۳ نسخه‌ی منتشرشده** از یک ماژول ورود ساده به یک
سامانه‌ی سازمانی کامل رسیده — از درخت تجهیزات و دستور کار تا نامه‌نگاری
اداری، انبار، تدارکات، امور کارکنان و گزارش‌های مدیریتی. جایگزین بومیِ یک
سامانه‌ی ERP خارجی شده و همان کار را با تقویم شمسی و زبان فارسی انجام
می‌دهد.

> **یک نکته‌ی صادقانه:** نسخه‌ی فعلی واژگان صنعت حفاری را دارد — واحد
> تفکیک دارایی‌ها «دکل» نامیده می‌شود. برای استقرار در صنعتی دیگر، این
> برچسب متناسب با همان صنعت («کارخانه»، «سایت»، «خط») تنظیم می‌شود؛
> ساختار و منطق زیرین تغییری نمی‌کند.
...
نیاز دارد که همراه دستور نصب به شما داده می‌شود. کاری با لایسنس سامانه
ندارد — لایسنس، *استفاده* را کنترل می‌کند و توکن فقط اجازه‌ی *دریافت
بسته* را می‌دهد.

توکن روی سرور در `/etc/davam/token` با دسترسی `600` و مالکیت `root`
می‌نشیند، نه در `.env` — چون `.env` را کانتینر برنامه هم می‌خواند و توکن
نباید در دسترس فرآیندی باشد که به درخواست‌های شبکه پاسخ می‌دهد.
